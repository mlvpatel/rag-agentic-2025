...
Covers: documented defaults, environment variable overrides, and yaml
config file overrides. Written before the implementation (TDD).
"""
...
def test_settings_has_documented_defaults():
    ...
    assert (
        ...
        == "postgresql://rag:rag@localhost:5432/rag_agentic"
    )
...
def test_yaml_file_overrides_defaults(tmp_path, monkeypatch):
    ...
    yaml_file.write_text(
        yaml.dump({"top_k": 12, "reranker_top_n": 8, "chunk_size": 500})
    )
...
def test_env_var_takes_precedence_over_yaml(tmp_path, monkeypatch):
    ...
    configs_dir.mkdir()
    yaml_file = configs_dir / "dev.yml"
    yaml_file.write_text(yaml.dump({"top_k": 12}))
